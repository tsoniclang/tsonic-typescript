...
export function excludeTargetProgramIndexNodes(
  program: TargetProgramIndex,
  excluded: (node: Node) => boolean,
): TargetProgramIndex {
  const nodes = Object.freeze(program.nodes.filter((node) => !excluded(node)));
  if (nodes.length === program.nodes.length) {
    return program;
  }
  const byFile = new Map<SourceFile, readonly Node[]>();
  const byKind = new Map<Kind, readonly Node[]>();
  const byKinds = new Map<string, readonly Node[]>();
  return Object.freeze({
    ...
    nodesFor(sourceFile: SourceFile): readonly Node[] {
      ...
      const selected = Object.freeze(
        program.nodesFor(sourceFile).filter((node) => !excluded(node)),
      );
      byFile.set(sourceFile, selected);
      return selected;
    },
    hasAuthoredIdentifierName(sourceFile: SourceFile, name: string): boolean {
      return program.hasAuthoredIdentifierName(sourceFile, name);
    },
    authoredIdentifierNameCount(sourceFile: SourceFile): number {
      return program.authoredIdentifierNameCount(sourceFile);
    },
    nodesOfKind(kind: Kind): readonly Node[] {
      ...
      const selected = Object.freeze(
        program.nodesOfKind(kind).filter((node) => !excluded(node)),
      );
      byKind.set(kind, selected);
      return selected;
    },
    nodesOfKinds(kinds: readonly Kind[]): readonly Node[] {
      const key = [...new Set(kinds)].sort((left, right) => left - right).join(",");
      const cached = byKinds.get(key);
      if (cached !== undefined) {
        return cached;
      }
      const selected = Object.freeze(
        program.nodesOfKinds(kinds).filter((node) => !excluded(node)),
      );
      byKinds.set(key, selected);
      return selected;
    },
    hasBindingWrite(declaration: Node | undefined): boolean {
      return program.hasBindingWrite(declaration);
    },
    bindingWritesAt(node: Node | undefined): readonly SourceBindingWrite[] {
      return program.bindingWritesAt(node);
    },
    bindingWritesFor(
      declaration: Node | undefined,
    ): readonly SourceBindingWrite[] {
      return program.bindingWritesFor(declaration);
    },
  });
}
